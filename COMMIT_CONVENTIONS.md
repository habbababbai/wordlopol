...
| ------------------- | ---------------- | ---------------- |
| `changelog-api.yml` | Manual (Actions) | `api-v0.x.x`     |
| `changelog-web.yml` | Manual (Actions) | `web-v0.x.x`     |

After merging releasable PR(s), run the changelog workflow for that app, then merge the **Release PR** from release-please. See [docs/CHANGELOG_AUTOMATION.md](./docs/CHANGELOG_AUTOMATION.md).
